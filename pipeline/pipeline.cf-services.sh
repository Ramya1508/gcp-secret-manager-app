...
if serviceMissing 'cal-service-registry' ; then
    cf create-service p.service-registry standard cal-service-registry
fi


if serviceEnabled "$CONFIGSERVER_ENABLED" && serviceEnabled "$CONFIGSERVER_CONFIGURED" && serviceMissing 'cal-config-server' ; then
cf create-service p.config-server standard cal-config-server -c "$(cat <<EOF
    {
        "git": {
...
if serviceMissing 'cal-rabbit' ; then
    cf create-service p.rabbitmq three-node-3.7 cal-rabbit
